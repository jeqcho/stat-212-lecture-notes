...
\emph{Stochastic Processes} $\{X_t : t \in \T\}$, $X_t \in \cS$ --- state space, $\T$ --- index set.

\textbf{Q.} Why care about stochastic processes? --- To model \& predict dynamics occurring in the physical world, e.g.\ weather dynamics, infection counts, stock prices, etc.

Interest: Can we predict future behavior? Can we use dynamics to produce ``samples'' of interest? --- applications in Gen AI.

\textbf{Our approach in this course:} Focus on Brownian Motion \& allied stochastic processes.

\textbf{Why?} BM is a canonical stochastic process with many fascinating properties. Using It\^o calculus, we can construct many other stochastic properties from BM.

\textbf{Finally:} The techniques \& principles we learn can be widely used to study other stochastic processes.

Before diving into a systematic study of BM, we have to build up some technical foundations.
